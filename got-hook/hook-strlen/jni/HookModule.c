...
#define LIBSF_PATH  "/data/app-lib/com.bradzhao.crackme-1/libtx.so"

size_t (*old_strlen)(const char *s) = -1;

size_t new_strlen(const char *s)
{
    char* result = old_strlen(s);
    if(result == NULL)
        return result;

    LOGD("[*] got table hook success ~");
        ...
        if (shdr.sh_type == SHT_PROGBITS) 
        {
            ...
            if (strcmp(&(string_table[name_idx]), ".got.plt") == 0 || strcmp(&(string_table[name_idx]), ".got") == 0) 
            {
                ...
                for (i = 0; i < out_size; i += 4) 
                {
                    ...
                    if (got_item  == old_strlen) 
                    {
                        LOGD("[*] Found strlen in got\n");    
                        got_found = 1;  
                        ...
                        // 替换为 new 函数地址
                        *(uint32_t *)(out_addr + i) = new_strlen;  
                        LOGD("[*] Hook success\n");    
                        break;      
                    }
                    else if (got_item == new_strlen) 
                    {
                        LOGD("[*] Already hooked\n");    
                        break;      
                    }

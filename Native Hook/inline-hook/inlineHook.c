...
#define PAGE_START(addr)	(~(PAGE_SIZE - 1) & (addr)) 	// 内存分页起始位置
#define SET_BIT0(addr)		(addr | 1)						// 设置最高位为1
#define CLEAR_BIT0(addr)	(addr & 0xFFFFFFFE)				// 最高位置0
#define TEST_BIT0(addr)		(addr & 1)						// 检查最高位是否为1
...
{
	LOGD("[*] start find inline hook item for hava target addr ~");
	...
	LOGD("[*] find inline hook item success ~");
...
{
	LOGD("[*] start find inline hook item ~");
...
{
	LOGD("[*] start register inline hook ~");
	...
	if (!isExecutableAddr(target_addr) || !isExecutableAddr(new_addr))
	{
		...
		LOGD("[*] Not an executable address!"); 
	}

	item = findInlineHookItem(target_addr);
	if (item != NULL) 
	{
		if (item->status == REGISTERED) 	// 注册
			return ELE7EN_ERROR_ALREADY_REGISTERED;		// 已注册
		else if (item->status == HOOKED) 	// hook
			return ELE7EN_ERROR_ALREADY_HOOKED;			// 已hook
		else 
			return ELE7EN_ERROR_UNKNOWN;				// 未知
	}
	...
	item->status = REGISTERED; 	// 状态置为已注册

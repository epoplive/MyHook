...
{    
    ...
    if (ptrace(PTRACE_GETREGSET, pid, (void*)regset, &ioVec) < 0) {    
        ...
        printf(" io 0x%08X, %d", ioVec.iov_base, ioVec.iov_len); 
        return -1;    
    }    
...
{    
    ...
    {    
        ...
        {    
            ...
            {    
                pch = strtok( line, "-" );          // 查找"-"出现位置
                addr = strtoull( pch, NULL, 16 );   // 转化基地址为无符号长整型 
...
{    
    ...
    // 遍历proc目录下进程id和process_name比较得到PID
...
{    
    ...
    DEBUG_PRINT("[+] Target process returned from %s, return value=0x%08X, pc=0x%08X \n", func_name, ptrace_retval(regs), ptrace_ip(regs));    
...
{    
    ...
    void* mmap_addr;        // mmap 映射进内存
    void* dlopen_addr;      // dlopen 打开动态链接库文件
    void* dlsym_addr;       // dlsym 获取函数或变量地址
    void* dlclose_addr;     // dlclose 关闭动态链接库
    void* dlerror_addr;     // dlerror 返回错误信息
    ...
    // 得到dlopen dlsym dlclose dlerrer 的函数地址
    dlopen_addr = get_remote_addr( target_pid, linker_path, (void *)dlopen );    
    dlsym_addr = get_remote_addr( target_pid, linker_path, (void *)dlsym );    
    dlclose_addr = get_remote_addr( target_pid, linker_path, (void *)dlclose );    
    dlerror_addr = get_remote_addr( target_pid, linker_path, (void *)dlerror );    
    
    DEBUG_PRINT("[+] Get imports: dlopen:  0x%08X\r\n", dlopen_addr);
    DEBUG_PRINT("[+] Get imports: dlsym:   0x%08X\r\n", dlsym_addr);
    DEBUG_PRINT("[+] Get imports: dlclose: 0x%08X\r\n", dlclose_addr);
    DEBUG_PRINT("[+] Get imports: dlerror: 0x%08X\r\n", dlerror_addr);
...
int main(int argc, char** argv) 
{
    ...
    if (-1 == target_pid) 
    {
        DEBUG_PRINT("[+] Can't find the process -> GAME0VER~ \n");
        return -1;  
    }  
